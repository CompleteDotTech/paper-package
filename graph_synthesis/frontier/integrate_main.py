...
def main():
    ...
    old="    anchor=next((x for x in ('<!-- SOURCE_STRUCTURAL_RESEARCH_END -->','<!-- STRUCTURAL_RESEARCH_END -->',ANCHOR) if x in text),ANCHOR)"
    new="    anchors=re.findall(r'<!-- [A-Z_]+_RESEARCH_END -->',text)\n    anchor=anchors[-1] if anchors else ANCHOR"
    if old in text:
        assert text.count(old)==1
        text=text.replace(old,new)
    else:
        assert new in text,'Unexpected paper anchor implementation'
    if NOTE not in text:
    ...
    note='Further integration preserves PR #24 at `'+PIN+'`, including its requirements and all evidence. The full regression runner includes its tests. The reporting anchor now follows the last existing research section, so the complete paper retains all three concurrent studies. Latent-source versus dependence-envelope assumptions and path-segment-tree versus tree-message update mechanisms are explicitly distinguished. No benchmark endpoint or implementation is changed.'
    if note not in text:
        path.write_text(text+'\n'+note+'\n',encoding='utf-8',newline='\n')
    ...
    print('Verified current main ancestry and three unchanged concurrent studies; frontier reporting remains additive')
