...
from graph_synthesis.visualize_evidence import (INPUTS, FIGURES, collect, dispositions,
    ...
    def test_canonical_tables_and_committed_data(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            for path in output.iterdir():
                self.assertEqual(path.read_bytes(), (ROOT / 'graph_synthesis/evidence_figures' / path.name).read_bytes())

    def test_svg_files_are_static_valid_and_accessible_text(self):
        for name in FIGURES:
            raw = (ROOT / 'graph_synthesis/evidence_figures' / (name + '.svg')).read_text(encoding='utf-8')
