# Adapted from the immutable original verifier: runtime/report roots and optional declared preflight only.
...
def verify(directory):
    ...
    require(len(probes) == int(bool(manifest.get("preflight_included_in_usage", False))) and
            all(p.get("purpose", "").startswith("toy authentication/schema probe") for p in probes),
            "Unexpected unplanned calls")
    ...
    report = {"status": "passed", "verified_at": datetime.now(timezone.utc).isoformat(), "verifier_sha256": sha(Path(__file__)),
              "run_directory": str(directory.relative_to(ROOT.parent)),
